...
class ODEblock(nn.Module):
  ...
  def set_tol(self):
    ...
    self.rtol = self.opt['tol_scale'] * 1e-7 #default is 1e-9
    if self.opt['adjoint']:
      self.atol_adjoint = self.opt['tol_scale_adjoint'] * 1e-7
      self.rtol_adjoint = self.opt['tol_scale_adjoint'] * 1e-7 #default is 1e-9

  def reset_tol(self):
    self.atol = 1e-7
    self.rtol = 1e-7 #default is 1e-9
    self.atol_adjoint = 1e-7
    self.rtol_adjoint = 1e-7 #default is 1e-9 
