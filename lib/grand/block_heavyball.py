#need to get a heavyball ODE block here
...
class HBNODEblock(ODEblock):
  def __init__(self, odefunc, regularization_fns, opt, data, device, t=torch.tensor([0, 1])):
    super(HBNODEblock, self).__init__(odefunc, regularization_fns, opt, data, device, t)
  ...
  def forward(self, x):
    ...
    #make input in tht form of  [h,m]
    state = torch.zeros(x.size()[0],2,x.size()[1])
    state[:,0] = (x,) + reg_states if self.training and self.nreg > 0 else x
    ...
    if self.opt["adjoint"] and self.training:
      ...
      state_dt = integrator(
        ...
        atol=self.atol,
        rtol=self.rtol)
    #now state_dt is in the form of [2,batch,2,feature] with dim=0 for [t0,tf], dim=2 for [h,m]
    if self.training and self.nreg > 0:
      z = state_dt[0,1,0]
      reg_states = tuple( st[1] for st in state_dt[1:] )
      return z, reg_states
    else: 
      z = state_dt[1,:,0] #take [tf,batch,h,feature]
      return z
