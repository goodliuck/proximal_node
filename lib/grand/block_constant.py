...
class ConstantODEblock(ODEblock):
  ...
  def forward(self, x):
    t = self.t.type_as(x)
    #self.training is a flag indicagting if this block is in training or not.
    ...
    else: #state is of size sample*features = 18333*16 at t=0, t is [0 T]
      state_dt = integrator(
        func, state, t,
        method=self.opt['method'],
        options=dict(step_size=self.opt['step_size'], max_iters=self.opt['max_iters']),
    ...
    if self.training and self.nreg > 0:
      ...
      z = state_dt[1] #t is just [0 T], so state_dt is of size timestamp*sample*features = 2*18333*16
      #import pdb;pdb.set_trace()
      return z
